...
function preload() {
  ...
  // Load background images (4320x2430 px)
...
function create() {
  ...
  // Large background with tileSprite for smooth scrolling
...
function update() {
  ...
  if(cursors.left.isDown){
    bg.tilePositionX -= speed*0.016;
    player.setVelocityX(-speed);
    player.anims.play(`${heroKey}_walk`,true);
    player.flipX=true;
    ...
    player.setVelocityY(cursors.up.isDown?-speed:speed);
    player.anims.play(`${heroKey}_walk`,true);
  } else if(heroKey==='hero4'){
    player.anims.play(`${heroKey}_fly`,true);
  } else {
    player.anims.play(`${heroKey}_idle`,true);
  }
